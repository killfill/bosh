  ...
  module Helpers
    ...
    def with_fifo
      ...
      begin
        ...
        overlimit = parse_fifo_response(e.response, "overLimit", "overLimitFault")
        unless overlimit.nil? || retries >= MAX_RETRIES
          ...
          @logger.debug("Fifo API Over Limit (#{details}), waiting #{wait_time} seconds before retrying") if @logger
          sleep(wait_time.to_i)
          retries += 1
          retry
        end
        cloud_error("Fifo API Request Entity Too Large error. Check task debug log for details.", e)
      rescue Excon::Errors::BadRequest => e
        badrequest = parse_fifo_response(e.response, "badRequest")
        details = badrequest.nil? ? "" : " (#{badrequest["message"]})"   
        cloud_error("Fifo API Bad Request#{details}. Check task debug log for details.", e)
      rescue Excon::Errors::InternalServerError => e
        unless retries >= MAX_RETRIES
          retries += 1
          @logger.debug("Fifo API Internal Server error, retrying (#{retries})") if @logger
          sleep(DEFAULT_RETRY_TIMEOUT)
          retry
        end
        cloud_error("Fifo API Internal Server error. Check task debug log for details.", e)
      end
    end
    
    ##
    # Parses and look ups for keys in an Fifo response 
    #
    # @param [Excon::Response] response Response from Fifo API
    # @param [Array<String>] keys Keys to look up in response
    # @return [Hash] Contents at the first key found, or nil if not found
    def parse_fifo_response(response, *keys)
    ...
    def wait_resource(resource, target_state, state_method = :status, allow_notfound = false)
      ...
      started_at = Time.now
      desc = resource.class.name.split("::").last.to_s + " `" + resource.uuid.to_s + "'"
      ...
      loop do
        ...
        if with_fifo { resource.reload.nil? }
          break if allow_notfound
          cloud_error("#{desc}: Resource not found")
        else
          puts "mimrame: #{state_method} #{resource}"
          state =  with_fifo { resource.send(state_method).downcase.to_sym }
        end
        ...
        # at the moment
        if resource.state.include?('failed')
