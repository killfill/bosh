...
module Bosh::FifoCloud
  ##
  # Represents Fifo server network config. Fifo server has single NIC
  ...
  # to make sure we don't apply something Fifo doesn't understand how to
  # deal with.
  class NetworkConfigurator
      ...
      spec.each_pair do |name, network_spec|
        ...
        case network_type
          ...
          when "vip"
            ...
            cloud_error("Invalid network type `#{network_type}': Fifo " \
    ...
    def configure(fifo, server)
      @network.configure(fifo, server)

      if @vip_network
        @vip_network.configure(fifo, server)
      else
